...
benchmark <- compiler::cmpfun(function(FUN, ..., sent.list, ext.output=T,options = list(optimize=3)) {
    require(stringi)
    require(digest)
    require(data.table)
...
benchmark(google_predictor_Level3, 
          # additional parameters to be passed to the prediction function can be inserted here
          sent.list = list('tweets' = tweets, 
                           'blogs' = blogs), 
          ext.output = T)

benchmark(Turing_Predictr_Level3, 
          # additional parameters to be passed to the prediction function can be inserted here
          sent.list = list('tweets' = tweets, 
                           'blogs' = blogs), 
          ext.output = T)
